...
class Settings(BaseSettings):
    ...
    # ================================================================
    # 一键拉取下载并发（kind=file 的 HTTP Range 分段并行）
    # ================================================================
    # 背景：单连接拉 6.7GB 模型在 VPN/代理链路下跑不满带宽，且一抖就
    # SSL UNEXPECTED_EOF 导致整文件重下；分段并行 + 段级续传是当前网络
    # 环境的最优解（gopeed 同款思路）。段数可按带宽/服务器限流策略调节；
    # 设为 1 即关闭分段（走原单流续传路径）。
    download_segments: int = Field(
        default=8,
        validation_alias=AliasChoices("NEXUS_DOWNLOAD_SEGMENTS", "DOWNLOAD_SEGMENTS"),
        description="kind=file 下载的并行分段数（默认 8，每段下限 64MiB；1=关闭分段）",
    )
